data "catalystcenter_credentials_cli" "cli_credentials" {
  for_each    = { for cli_cred in try(local.catalyst_center.network_settings.device_credentials.cli_credentials, [{ name = local.defaults.catalyst_center.network_settings.device_credentials.cli_credentials.name }]) : cli_cred.name => cli_cred }
