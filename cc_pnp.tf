...
resource "catalystcenter_pnp_device_claim_site" "claim_device" {
  ...
  config_id         = try(catalystcenter_template.regular_template[each.value.onboarding_templates.regular[0].name].id, null)
  config_parameters = try(each.value.onboarding_templates.regular[0].variables, local.defaults.catalyst_center.onboarding_templates.variables, null)
