...
resource "catalystcenter_pnp_device_claim_site" "claim_device" {
  ...
  config_id         = catalystcenter_template.regular_template[local.onboarding_template_sites[each.value.site].onboarding_template].id
  config_parameters = try(each.value.onboarding_templates.variables, local.defaults.catalyst_center.onboarding_templates.variables, null)
