...
resource "catalystcenter_wireless_profile" "wireless_profile" {
  ...
  ssid_details = try([for ssid in each.value.ssid_details : {
    ...
    interface_name      = try(each.enable_fabric, false) == true ? try(ssid.interface_name, local.defaults.catalyst_center.network_profiles.wireless.ssid_details.interface_name, null) : null
  }], null)
