...
resource "catalystcenter_wireless_ssid" "ssid" {
  ...
  multi_psk_settings = length(try(each.value.multi_psk_settings, local.defaults.catalyst_center.wireless.ssids.multi_psk_settings, [])) > 0 ? [
    for p in try(each.value.multi_psk_settings, local.defaults.catalyst_center.wireless.ssids.multi_psk_settings, []) : {
      priority              = try(p.priority, null)
      passphrase_type       = try(p.passphrase_type, null)
      passphrase_wo         = sensitive(try(p.passphrase, null))
      passphrase_wo_version = try(p.passphrase_version, 1)
    }
  ] : null
  ...
  passphrase_wo              = sensitive(try(each.value.passphrase, local.defaults.catalyst_center.wireless.ssids.passphrase, null))
