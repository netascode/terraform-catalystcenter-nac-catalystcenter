...
**Breaking Changes:**
- Require `inventory.devices[].type: WirelessController` on non-embedded wireless controllers (standalone and fabric `WIRELESS_CONTROLLER_NODE`); such a device with managed AP locations or a wireless-controller fabric role that omits it is rejected at plan time. Embedded controllers (`EMBEDDED_WIRELESS_CONTROLLER_NODE`) are unaffected
