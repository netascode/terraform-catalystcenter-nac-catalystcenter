...
- Configure access points from a new optional `inventory.devices[].access_point` block; a device participates when it carries that block or an `access_point_configuration` reference. Supports admin status, AP mode, failover priority, LED status and brightness, location, the primary/secondary/tertiary controller trio, and per-radio settings (role, admin status, antenna pattern and gain, antenna cable and loss, channel and channel width, power assignment mode and level). The data model uses human-readable values (`ap_mode: LOCAL`, `channel_width: 80`, `band: "2.4"`) which the module translates to the API's integer encodings, and the API's `configure_*` flags are derived from value presence
- Add named access point configurations under `wireless.access_point_configurations`, referenced from a device by `inventory.devices[].access_point_configuration`, so shared radio and controller settings can be declared once. The Configure Access Points API applies every setting except the hostname to the whole `ap_list`, so access points are grouped by identical configuration and one API call is issued per group. Effective settings resolve the inline `access_point` block first, then the named configuration, then `defaults.catalyst_center.inventory.devices.access_point`. Plan-time validation rejects undefined configuration references, missing ethernet MACs, conflicting location and role/RF settings, explicit channel/power values without a matching `CUSTOM` assignment mode, a channel width on 2.4 GHz, and out-of-range antenna gain or cable loss
- Reconcile access point hostnames to `inventory.devices[].name` via `inventory.devices[].access_point.manage_hostname` (default `false`, settable per device or once under `defaults`). Access points are resolved by `serial_number` and identified to the API by their ethernet MAC, so a rename does not break identification on the next apply and renaming to the name already held is a no-op
