...
- Configure access points from named configurations declared under `wireless.access_point_configurations` and referenced from a device by `inventory.devices[].access_point.configuration`. A configuration supports admin status, AP mode, failover priority, LED status and brightness, location, the primary/secondary/tertiary controller trio, and per-radio settings (role, admin status, antenna pattern and gain, antenna cable and loss, channel and channel width, power assignment mode and level). The data model uses human-readable values (`ap_mode: LOCAL`, `channel_width: 80`, `band: "2.4"`) which the module translates to the API's integer encodings, and the API's `configure_*` flags are derived from value presence. Settings resolve the named configuration first, then `defaults.catalyst_center.wireless.access_point_configurations`. Settings are never declared on the device itself: the Configure Access Points API applies every setting except the hostname to the whole `ap_list`, so a per-device override would silently fork that access point into an API call of its own. One API call is issued per named configuration, and `for_each` is keyed by the configuration name so plan output stays readable. Applying a configuration resolves the access point by `serial_number`, `name` or `fqdn_name`
- Reconcile access point hostnames to `inventory.devices[].name` via `inventory.devices[].access_point.manage_hostname` (default `false`, settable per device or once under `defaults`). Unlike applying a configuration, a rename requires `serial_number`: renaming changes the hostname the access point would be found by, so a name-based lookup stops resolving the moment it succeeds. The desired name is sent unconditionally rather than derived from a comparison against the live hostname, which would make the resource mutate its own arguments and cost a second apply to settle; the API identifies the access point by its ethernet MAC, and renaming to the name already held is a no-op. Access points that manage a hostname without referencing a configuration are grouped under a reserved `(hostname only)` key
- Validate access point configurations at plan time under one rule: a `precondition` fails the plan when Catalyst Center would reject the request or return an undefined result — undefined configuration references, `manage_hostname` without a `serial_number`, conflicting `location` and `is_assigned_site_as_location`, channel or power settings on a non-`SERVING` radio (NCWL10977), explicit channel/width/power without a matching `CUSTOM` assignment mode (NCWL10959), and a channel width on 2.4 GHz (NCWL10970). A `check` warns without blocking when Catalyst Center accepts the request and silently ignores the value — `antenna_gain` without `antenna_pattern_name: other`, `cable_loss` without `antenna_cable_name: other` — or when an access point can be skipped without affecting the others, which is the case for an access point that has no ethernet MAC yet because it has not been claimed. Configuration-scoped errors name the configuration once rather than enumerating every device that references it
