...
**Bug Fixes:**
- Fix device credential objects failing to delete with `NCIM01100` when they are still assigned to a site. Requires provider `~> 0.5.18`, where `catalystcenter_assign_credentials` resource now unassigns credentials when they are removed from the data model.
...
- Fix `catalystcenter_pnp_device_claim_site` hostname resolution; the claim hostname now uses the device `name` (falling back to `fqdn_name`, then the `pnp.devices.hostname` default), instead of an explicit per-device `hostname` override. This ensures the inventory hostname/AP name matches the device `name` defined in the data model.
