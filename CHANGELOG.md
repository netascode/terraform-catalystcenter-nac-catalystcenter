...
- Reconcile access point hostnames to `inventory.devices[].name` via `manage_ap_hostname` (default `false`). Access points are resolved by `serial_number`, so a rename does not break identification on the next apply.
