...
- Configure access points from a new optional `inventory.devices[].access_point` block. A device participates when it carries that block or an `access_point_configuration` reference, so the opt-in lives in the data model rather than in a module variable. Supports admin status, AP mode, failover priority, LED status and brightness, location, the primary/secondary/tertiary controller trio, and per-radio settings (role, admin status, antenna pattern and gain, antenna cable and loss, channel and channel width, power assignment mode and level). The data model uses human-readable values (`ap_mode: LOCAL`, `failover_priority: HIGH`, `channel_width: 80`, `band: "2.4"`) which the module translates to the API's integer encodings, and the API's `configure_*` flags are derived from value presence rather than being exposed in the data model.
- Add named access point configurations under `wireless.access_point_configurations`, referenced from a device by `inventory.devices[].access_point_configuration`. They are declared alongside the other wireless constructs because they describe reusable radio and controller intent, while `inventory` keeps the per-device records that consume them. Because the Configure Access Points API applies every setting except the hostname to the whole `ap_list`, access points must be grouped by identical configuration and one API call issued per group; naming those groups explicitly bounds their number by design and keeps `for_each` keys stable and readable in plan output instead of opaque payload hashes. Effective settings resolve inline `access_point` block first, then the referenced named configuration, then `defaults.catalyst_center.inventory.devices.access_point`, so shared settings can be declared once. A device that adds an inline override no longer shares its group with the plain users of the same named configuration. Devices referencing an undefined configuration are reported by an `access_point_configuration_reference_validation` check.
- Reconcile access point hostnames to `inventory.devices[].name` via `inventory.devices[].access_point.manage_hostname` (default `false`, settable per device or once under `defaults`). Access points are resolved by `serial_number`, so a rename does not break identification on the next apply. The desired name is sent unconditionally rather than being derived from a comparison against the access point's live hostname: deriving it from the inventory data source would make the resource's own arguments change as a side effect of a successful rename, costing a second apply to settle and breaking idempotency. Renaming an access point to the name it already holds is a no-op, and the API identifies the access point by its ethernet MAC, so the current hostname is never sent.
