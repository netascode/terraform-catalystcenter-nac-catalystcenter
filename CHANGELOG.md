...
- Validate access point configurations at plan time: a `precondition` fails the plan where Catalyst Center would reject the request or return an undefined result (undefined configuration reference, `manage_hostname` without `serial_number`, `location` conflicting with `is_assigned_site_as_location`, NCWL10977, NCWL10959, NCWL10970); a `check` warns where a value is silently ignored (`antenna_gain` or `cable_loss` without the matching `other` pattern) or an access point is skipped for having no ethernet MAC yet
