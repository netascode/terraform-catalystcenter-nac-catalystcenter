...
**Bug Fixes:**
- Fix device credential objects failing to delete with `NCIM01100` when they are still assigned to a site. Requires provider `~> 0.5.18`, whose `catalystcenter_assign_credentials` resource now unassigns credentials when they are removed from the data model: it sends `null` for each removed credential slot — on an in-place update only the removed slot(s), keeping the others; on destroy every slot the assignment manages. Sending `null` removes the site-local assignment so the slot falls back to whatever is inherited from a parent site (rather than leaving a stale assignment), which clears the association so the credential definition can be deleted; slots that were only inherited from a parent are left untouched. (At the global site, which has no parent to inherit from, those slots are cleared/unset instead.) Removing a credential together with its entire site assignment now succeeds in a single apply, because Terraform destroys the assignment before the credential object. Removing only **one** credential from a site while keeping the others assigned there cannot be completed in a single apply due to a Terraform ordering limitation (it does not guarantee the assignment update runs before the credential delete); do it in two applies instead — first remove the credential from the site's `device_credentials` assignment, then delete the credential definition.
